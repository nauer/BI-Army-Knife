...
# encoding: utf-8
'''
linesplit -- split lines by a specific regular expression pattern from std-output or file and create for each hit additional new lines. 
...
def start(args):
    ...
    for line in args.file:
        ...
        if result:
            ...
            results = results + [l.strip() for l in result if len(l.strip()) > 0]
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        parser.add_argument('pattern', nargs=1, help="Split by pattern. Use | to separate more than one delimiter i.e. ';|,'",type=str)
        ...
        parser.add_argument('-d', '--delimiter', help='Set the delimiter for option group',type=str, default='\t')
        ...
        parser.add_argument('-g', '--group', help='Instead of normal input identical lines are grouped together and an additional column is added with the group count separated by the defined delimiter -d.', action='store_true')
